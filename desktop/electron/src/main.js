...
ipcMain.handle('dict:load-local', async (event, filePath) => {
  return getNative().loadLocalDictionary(String(filePath), (_err, progress) => {
...
ipcMain.handle('dict:lookup-resource', (_event, dictid, key) => getNative().lookupResource(dictid, key))
